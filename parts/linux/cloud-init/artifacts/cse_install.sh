...
downloadCredentialProvider() {
    ...
    # For the custom credential provider override, skip NI ACR rewriting and fall
    # through to the isRegistryUrl branch so we always pull directly from the
    # original registry (jinzha1.azurecr.io).
    if [ -n "${BOOTSTRAP_PROFILE_CONTAINER_REGISTRY_SERVER}" ] && ! isRegistryUrl "${CREDENTIAL_PROVIDER_DOWNLOAD_URL}"; then
